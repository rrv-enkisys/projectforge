...
resource "google_artifact_registry_repository" "docker" {
  ...
  format        = "DOCKER"
  project       = local.project_id
  description   = "ProjectForge Docker images"
...
module "secrets" {
  ...
  # Sensitive values — pass via TF_VAR_* or secrets.tfvars (never commit)
...
module "api_gateway" {
  ...
  allow_public          = false # Traffic comes through the Load Balancer only
